    ...
    <div className="p-4">
      ...
      >
        {months.map((month, index) => (
          <Card key={month} className="bg-[#081028] text-white shadow-lg rounded-lg">
            <CardContent className="p-4">
              <h2 className="text-lg font-semibold text-center mb-2">{month} 2025</h2>
              <div className="grid grid-cols-7 gap-1 text-sm">
                {["S", "M", "T", "W", "T", "F", "S"].map((d) => (
                  <span key={d} className="text-gray-400 text-center">{d}</span>
                ))}
                {generateDays(2025, index).map((date, i) => (
                  <motion.div
                    key={i}
                    className={`w-8 h-8 flex items-center justify-center rounded-full ${
                      selectedDates.includes(date) ? "bg-purple-500 text-white" : "text-gray-300"
                    }`}
